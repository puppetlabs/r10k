...
on(master, "#{r10k_fqp} deploy environment --modules --verbose debug")
...
on(master, "#{r10k_fqp} puppetfile purge --puppetfile #{remote_puppetfile_path} --moduledir #{moduledir} --verbose debug")
