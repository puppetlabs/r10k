...
module R10K
  class Tarball
    ...
    # @!attribute [r] cache_path
    #   @return [Pathname] Where the tarball will be cached to.
    ...
    def initialize(name, source, sha256digest: nil)
      ...
      @cache_path = Pathname.new(File.join(cache_root, tarball_basename))
    end
    ...
    def http_get(uri, method: Net::HTTP::Get, redirect_limit: 10, &block)
      ...
      px_ho, px_po, px_us, px_pa = if settings[:proxy]
      ...
      connection = Net::HTTP.new(uri.host, uri.port, px_ho, px_po, px_us, px_pa)
