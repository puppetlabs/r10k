...
require 'r10k/hg/working_repository'
require 'forwardable'
...
class R10K::Hg::StatefulRepository
  ...
  def initialize(rev, remote, basedir, dirname)
    @rev = rev
    @remote = remote

    @cache = R10K::Hg::Cache.generate(remote)
    @repo = R10K::Hg::WorkingRepository.new(basedir, dirname)
  end
  ...
  def sync
    ...
    if sha.nil?
      raise R10K::Hg::UnknownRevisionError.new("Unable to sync repo to unresolvable revision '#{@rev}'", :hg_dir => @repo.hg_dir)
    end

    case status
      when :absent
        logger.debug { "Cloning #{@repo.path} and checking out #{@rev}" }
        @repo.clone(@cache.hg_dir.to_s, {:rev => sha})
  ...
  def status
    if !@repo.exist?
      :absent
    elsif !@repo.hg_dir.exist?
      :mismatched
    elsif !@repo.hg_dir.directory?
      :mismatched
